...
function DeltaItem({ path, filePath, originalValue, value, timestamp, diff }: DeltaItemProps) {
  const date = new Date(timestamp);
  const formattedDate = `${date.toLocaleDateString()} ${date.toLocaleTimeString()}`;
  
  return (
    <Box flexDirection="column" borderStyle="round" padding={1} marginBottom={1}>
      <Text bold>Path: <Text color="blue">{path}</Text></Text>
      <Text>File: <Text color="yellow">{path.basename(filePath)}</Text></Text>
...
export default function DeltaLog() {
  ...
  const handleScroll = (direction: 'up' | 'down') => {
    ...
    } else if (direction === 'down' && scrollIndex < sortedEdits.length - 1) {
      setScrollIndex(scrollIndex + 1);
    }
  };
  ...
  const visibleEdits = sortedEdits.slice(scrollIndex, scrollIndex + visibleItems);
  
  return (
    <Box flexDirection="column" height="100%">
      <Box flexDirection="column" flexGrow={1}>
        <Text bold>JSON Edit History</Text>
        <Text color="gray">Total edits: {edits.length}</Text>
